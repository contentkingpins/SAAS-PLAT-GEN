...
// Helper function to generate unique ID (similar to cuid)
function generateId() {
...
async function importBaselineData() {
  console.log('Starting baseline data import with direct PostgreSQL connection...');
  ...
  try {
    ...
    // First, check if we have a default vendor or create one
    let vendorId;
    const vendorQuery = 'SELECT id FROM "Vendor" WHERE "code" = $1 AND "isActive" = true LIMIT 1';
    const vendorResult = await client.query(vendorQuery, ['BASELINE']);
    
    if (vendorResult.rows.length === 0) {
      // Create a default baseline vendor
      const insertVendorQuery = `
        INSERT INTO "Vendor" (id, name, code, "staticCode", "isActive", "createdAt", "updatedAt")
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id
      `;
      const vendorId_new = generateId();
      const now = new Date();
      
      const vendorInsertResult = await client.query(insertVendorQuery, [
        vendorId_new,
      ...
      console.log('Using existing baseline vendor');
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        ...
        while (mbiExists && attempts < 10) {
          mbi = generateMBI();
          const checkMbiQuery = 'SELECT id FROM "Lead" WHERE mbi = $1 LIMIT 1';
          const mbiResult = await client.query(checkMbiQuery, [mbi]);
          mbiExists = mbiResult.rows.length > 0;
          attempts++;
        }
        ...
        await client.query(insertLeadQuery, [
          ...
          'BASELINE',
          'SUBMITTED',
          normalizedTestType && ['IMMUNE', 'NEURO'].includes(normalizedTestType) ? normalizedTestType : null,
          false,
          false,
          0,
          now,
    ...
    if (errors.length > 0) {
      console.log('\n=== Error Details ===');
      errors.slice(0, 10).forEach(error => console.log(error)); // Show first 10 errors
      if (errors.length > 10) {
